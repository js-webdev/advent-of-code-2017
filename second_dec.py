...
TEST_STRING_MINMAX = """5 1 9 5
7 5 3
2 4 6 8"""

TEST_STRING_DIV = """5 9 2 8
9 4 7 3
3 8 6 5"""

MY_STRING = """1208	412	743	57	1097	53	71	1029	719	133	258	69	1104	373	367	365
...
def sum_spreadsheet(spreadsheet):
    row_arr = spreadsheet.split("\n")
    diff_list = []
    for row in row_arr:
        diff_list.append(row_diff(row))
    print(sum(diff_list))
...
def div_spreadsheet(spreadsheet):
    row_arr = spreadsheet.split("\n")
    div_list = []
    for row in row_arr:
        div = row_divide(row)
        if div != False:
            div_list.append(row_divide(row))
        else:
            print("NOOO")
    print(sum(div_list))

sum_spreadsheet(TEST_STRING_MINMAX)
sum_spreadsheet(MY_STRING)

div_spreadsheet(TEST_STRING_DIV)
div_spreadsheet(MY_STRING)
